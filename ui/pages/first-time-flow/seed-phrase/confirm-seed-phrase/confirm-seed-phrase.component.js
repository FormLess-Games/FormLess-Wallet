...
export default class ConfirmSeedPhrase extends PureComponent {
  ...
  isValid() {
    ...
    const seedPhraseStr = selectedSeedIndices
      .map((item) => item.split('-')[0])
      .join(' ');
    return seedPhrase === seedPhraseStr;
  }

  handleSetSeedIndices = (item, index) => {
    const { selectedSeedIndices } = this.state;
    const { seedPhrase } = this.props;
    const i = selectedSeedIndices.findIndex(
      (items) => `${item}-${index}` === items,
    );

    if (i === -1) {
      this.setState({
        selectedSeedIndices: [...selectedSeedIndices, `${item}-${index}`],
      });
      if (
        [...selectedSeedIndices, item].length === 12 &&
        seedPhrase !==
          [...selectedSeedIndices, item]
            .map((item) => item.split('-')[0])
            .join(' ')
      ) {
      ...
      selectedSeedIndices.splice(i, 1);
      this.setState({
  ...
  render() {
    ...
    return (
      <div className="confirm-seed-phrase">
        ...
        <div className="first-time-flow__list">
          {sortedSeedWords.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className={`${iIdex ? '' : 'an'} first-time-flow__li`}
                onClick={() => this.handleSetSeedIndices(item, index)}
              >
                <span>{iIdex}</span>
                <em>{item}</em>
              </div>
        ...
        >
          {this.context.t('complateGoWallet')}
        </Button>

        {isValidError && (
          <div className="rs-err-valid">
            <img src="./images/error.png" />
            <span>{this.context.t('validationError')}</span>
          </div>
        )}
      </div>
    );
