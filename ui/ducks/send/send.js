...
import EthUtil, { addHexPrefix, toChecksumAddress } from 'ethereumjs-util';
...
export function updateNFTSendAsset({ type, details, balance }) {
  return async (dispatch, getState) => {
    const state = getState();
    ...
    await dispatch(actions.updateNFTAsset({ type, details, balance }));
    // await dispatch(computeEstimatedGasLimit());
  };
}
...
export function signNFTTransaction(data) {
  return async (dispatch, getState) => {
    const state = getState();
    const {
      asset,
    ...
    try {
      const token = global.eth.contract(nftAbi).at(data.contractAddress);
      if (data.states === '721') {
        token
          .transferFrom(data.selectedAddress, data.toAddress, data.tokenId, {
            ...txParams,
          })
          .then((res) => {
            console.log(res);
