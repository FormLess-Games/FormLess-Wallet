    ...
    <WalletOverview
      ...
      buttons={
        <div className="eth-overview-btn-list">
          <div onClick={() => history.push(SEND_ROUTE)}>{t('send')}</div>
          <div onClick={() => dispatch(showModal({ name: 'ACCOUNT_DETAILS' }))}>
            {t('receive')}
          </div>
          <div onClick={() => window.open('https://paxful.com/')}>
            {t('fiatRecharge')}
          </div>
        </div>
        // <>
      ...
      className={className}
      icon={<Identicon diameter={50} image={primaryTokenImage} imageBorder />}
    />
  );
};
