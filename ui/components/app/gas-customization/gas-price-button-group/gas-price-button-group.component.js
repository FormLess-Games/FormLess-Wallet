...
export default class GasPriceButtonGroup extends Component {
  ...
  handInitGasLimit() {
    const { isNFT, updateGasLimit } = this.props;
    if (isNFT) {
      updateGasLimit('0x32039');
    }
  }

  handleTimeoutInitGasLimit() {
    const { buttonDataLoading } = this.props;
    if (!buttonDataLoading) {
      this.handInitGasLimit();
    } else {
      setTimeout(() => {
        this.handleTimeoutInitGasLimit();
      }, 50);
    }
  }
