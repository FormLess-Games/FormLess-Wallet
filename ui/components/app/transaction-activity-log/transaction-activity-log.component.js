...
export default class TransactionActivityLog extends PureComponent {
  ...
  handleActiveType(type) {
    switch (type) {
      case 'transactionCreated':
        return [
          this.context.t('transactionType1'),
          this.context.t('transactionType2'),
        ];
      case 'transactionSubmitted':
        return [
          this.context.t('transactionType3'),
          this.context.t('transactionType4'),
        ];
      case 'transactionResubmitted':
        return [
          this.context.t('transactionType5'),
          this.context.t('transactionType6'),
        ];
      case 'transactionConfirmed':
        return [this.context.t('transactionType7')];
      case 'transactionErrored':
        return this.context.t('transactionType8');
      case 'transactionDropped':
        return this.context.t('transactionType9');
      case 'transactionCancelAttempted':
        return [
          this.context.t('transactionType10'),
          this.context.t('transactionType11'),
        ];
      case 'transactionCancelSuccess':
        return [this.context.t('transactionType12')];
      default:
        return [];
    }
  }
  ...
  renderActivity(activity, index) {
    ...
    const activityType = this.handleActiveType(eventKey);
    return (
      <div key={index} className="transaction-activity-log__activity">
        ...
        <div className="transaction-activity-log__entry-container">
          ...
          >
            <div className="rs-activity-text">
              {activityType[0] && (
                <div className="cont">
                  <div className="title">{activityType[0]}</div>
                  <div className="time">{formattedTimestamp}</div>
                </div>
              )}
              {activityType[1] && (
                <div className="cont an">
                  <div className="title">{ethValue}</div>
                  <div className="time">{activityType[1]}</div>
                </div>
              )}
            </div>
            {/* {activityText} */}
