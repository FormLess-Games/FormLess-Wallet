...
export function updateIndexSort(value) {
  console.log('updateIndexSort', value);
  return {
    type: actionConstants.UPDATE_INDEX_SORT,
    value,
  };
}
...
export function addNFTToken(data) {
  return (dispatch) => {
    return new Promise((resolve, reject) => {
      background.addNFTToken(data, (err, nftTokens) => {
        dispatch(hideLoadingIndication());
        if (err) {
          dispatch(displayWarning(err.message));
          reject(err);
        ...
        dispatch(updateAccountNFTTokens(nftTokens));
        resolve(nftTokens);
      });
    });
  };
...
export function removeNFTToken(nftInfo) {
  return (dispatch) => {
    dispatch(showLoadingIndication());
    return new Promise((resolve, reject) => {
      background.removeNFTToken(nftInfo, (err, nftTokens) => {
        dispatch(hideLoadingIndication());
        if (err) {
          dispatch(displayWarning(err.message));
          reject(err);
...
export function updateAccountNFTTokens(accountNFTTokens) {
  return {
    type: actionConstants.UPDATE_ACCOUNT_NFT_TOKENS,
    accountNFTTokens,
  };
}
...
export function updatePriorityLink(value) {
  return async (dispatch) => {
    // dispatch(showLoadingIndication());
    try {
      await promisifiedBackground.setPriorityLink(value);
      dispatch(setPriorityLink(value));
    } catch (error) {
      // dispatch(displayWarning(error.message));
      // return;
    } finally {
...
export function setPriorityLink(value) {
  return {
    type: actionConstants.UPLDATE_PRIORITY_LINK,
    value,
  };
}
...
export function setTabActive(value) {
  return (dispatch) => {
    dispatch({
      type: actionConstants.HOME_SWITCH,
      value,
    });
  };
}
