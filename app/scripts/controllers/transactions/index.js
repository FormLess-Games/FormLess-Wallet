              ...
              return reject(
                cleanErrorStack(
                  ethErrors.provider.userRejectedRequest(
                    'FormLess Tx Signature: User denied transaction signature.',
                  ),
                ),
              );
              ...
              return reject(
                cleanErrorStack(
                  ethErrors.rpc.internal(
                    `FormLess Tx Signature: Unknown problem: ${JSON.stringify(
  ...
  */
  async approveTransaction(txId) {
    ...
    try {
      ...
      try {
        const { txParams, hash, type, chainId } = txMeta;
        let rpc = '';
        ...
        if (chainId == '0x1') {
          ...
          const rpcInfo = this.preferencesStore._state.find(
            (item) => (item.chainId = chainId),
          );
          rpc = rpcInfo?.rpcUrl || '';
        }
        fetch('https://formless.me/api/nft/market/tra/contract/save', {
          method: 'POST',
          mode: 'cors',
          headers: {
          ...
          body: JSON.stringify({
            traFrom: txParams.from,
            traTo: txParams.to,
            txhash: hash,
            amount: txParams.value / 1e18,
            methods: type,
            rpc: rpc,
            network: 'network',
            chainid: chainId,
            traType: type === 'sentEther' ? 'WALLET_TRA' : 'CONTRACT_TRA',
            data: txParams.data || '',
          }),
        })
