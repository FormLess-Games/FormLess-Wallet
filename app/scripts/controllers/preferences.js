...
export default class PreferencesController {
  ...
  setCurrentLocale(key) {
    console.log('setCurrentLocale', key);
  ...
  setPriorityLink(value) {
    console.log('setPriorityLink', value);
    this.store.updateState({
      priorityLink: value,
    });
    return value;
  ...
  async removeNFTToken(nftInfo) {
    const { accountNFTTokens } = this.store.getState();
    const { selectedAddress } = this.store.getState();
    const chainId = this.network.getCurrentChainId();
    const nftTokens = accountNFTTokens[selectedAddress][chainId];
    const previousIndex = nftTokens.findIndex(
    ...
    nftTokens.splice(previousIndex, 1);
    accountNFTTokens[selectedAddress][chainId] = nftTokens;

    this.store.updateState({
      accountNFTTokens,
    });
    return Promise.resolve(accountNFTTokens);
  }

  async addNFTToken(nftInfo) {
    const { accountNFTTokens } = this.store.getState();
    const { selectedAddress } = this.store.getState();
    const chainId = this.network.getCurrentChainId();
    ...
    !accountNFTTokens[selectedAddress] &&
      (accountNFTTokens[selectedAddress] = {});
    !accountNFTTokens[selectedAddress][chainId] &&
      (accountNFTTokens[selectedAddress][chainId] = []);
    const nftTokens = accountNFTTokens[selectedAddress][chainId];
    ...
    if (previousIndex === -1) {
      nftTokens.unshift(nftInfo);
    } else {
      nftTokens[previousIndex] = nftInfo;
    }
  ...
  _getNFTTokenRelatedStates(selectedAddress) {
    const { accountNFTTokens } = this.store.getState();
    if (!selectedAddress) {
      // eslint-disable-next-line no-param-reassign
      selectedAddress = this.store.getState().selectedAddress;
    ...
    const chainId = this.network.getCurrentChainId();
    if (!(selectedAddress in accountNFTTokens)) {
      accountNFTTokens[selectedAddress] = {};
    }
    if (!(chainId in accountNFTTokens[selectedAddress])) {
      accountNFTTokens[selectedAddress][chainId] = [];
    }
    const nftTokens = accountNFTTokens[selectedAddress][chainId];
    return {
