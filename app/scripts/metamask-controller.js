...
import { PROVIDER } from './constants/connect';
...
export default class MetamaskController extends EventEmitter {
  ...
  constructor(opts) {
    ...
    this.txController = new TransactionController({
      ...
      // signNFTTransaction: this.keyringController.signNFTTransaction.bind(
      //   this.keyringController,
      // ),
      provider: this.provider,
      blockTracker: this.blockTracker,
  ...
  initializeProvider() {
    ...
    const providerOpts = {
      static: {
        eth_syncing: false,
        web3_clientVersion: `FormLess/v${version}`,
      },
      version,
      // account mgmt
   ...
   *
   * @param {Object} [memState] - The FormLess memState. If not provided,
  ...
  getApi() {
    ...
    return {
      ...
      setPriorityLink: this.setPriorityLink.bind(this),
      ...
      addNFTToken: nodeify(
        preferencesController.addNFTToken,
        preferencesController,
      ),
      removeNFTToken: nodeify(
        preferencesController.removeNFTToken,
        preferencesController,
      ),
  ...
  setupUntrustedCommunication(connectionStream, sender) {
    ...
    if (usePhishDetect && this.phishingController.test(hostname)) {
      log.debug('FormLess - sending phishing warning for', hostname);
  ...
  /**
   * A method for emitting the full FormLess state to all registered listeners.
   * @private
   */
  privateSendUpdate() {
  ...
  setPriorityLink(key, cb) {
    try {
      const direction = this.preferencesController.setPriorityLink(key);
      cb(null, direction);
      return;
    } catch (err) {
      cb(err);
  ...
  /**
   * A method for recording whether the FormLess user interface is open or not.
   * @private
   * @param {boolean} open
   */
